...
void main() {
  ...
  group('estados nunca mentem sobre o envio', () {
    test('nova tentativa não reabre ocorrência já recebida', () async {
      final received = buildOccurrence().copyWith(
        status: QueueStatus.receivedByCentral,
        protocol: 'BLU-20260909-AAAAAAAA',
        receivedAt: now,
      );
      await store.insert(received);
      await buildQueue().retryNow(received.id);
      final saved = await store.byId(received.id);
      expect(saved!.status, QueueStatus.receivedByCentral);
      expect(saved.protocol, received.protocol);
