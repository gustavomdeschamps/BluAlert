...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: media, error: mediaError } = await client.from('occurrence_media').select('*').eq('occurrence_id', occurrence.id);
    ...
    for (const item of media) {
      ...
      const { data: content, error: downloadError } = await client.storage.from('occurrence-media').download(item.object_path);
      if (downloadError || !content) return json({ error: 'UPLOAD_INCOMPLETE' }, 409, cors);
      const digest = await crypto.subtle.digest('SHA-256', await content.arrayBuffer());
      const hash = Array.from(new Uint8Array(digest), byte => byte.toString(16).padStart(2, '0')).join('');
      if (hash !== item.sha256) return json({ error: 'INVALID_MEDIA' }, 409, cors);
    }
