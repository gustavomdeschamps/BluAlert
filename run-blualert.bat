...
set "BLUALERT_TEST_ARGS="
if /i "%~1"=="--test" set "BLUALERT_TEST_ARGS=--dart-define=TEST_LOCATION_ENABLED=true"
...
call flutter run -d chrome %BLUALERT_TEST_ARGS%
exit /b %errorlevel%
...
echo Abrindo BluAlert no Chrome...
call flutter run -d chrome --dart-define-from-file=.dart-defines.json %BLUALERT_TEST_ARGS%
