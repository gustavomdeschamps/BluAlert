...
class BackendClient {
  ...
  Future<BackendSession> _refresh(String refreshToken) async {
    ...
    if (response.statusCode == 400 ||
      ...
      throw const _InvalidRefreshToken();
    }
    if (response.statusCode < 200 || response.statusCode >= 300) {
      throw const BackendUnavailable(
          'Não foi possível renovar o acesso. Tente novamente quando houver conexão.');
    }
  ...
  void _requireConfiguration() {
    if (!isConfigured) {
      throw const BackendUnavailable(
        'Este aplicativo foi aberto sem a configuração do Supabase. Configure o projeto e reinicie o app; sua conta não precisa ser cadastrada novamente.',
      );
    }
  }
