...
function Map({ items, selected, onSelect }: { items: QueueItem[]; selected: string | null; onSelect: (id: string) => void }) {
  ...
  useEffect(() => {
    ...
    if (!item || !mapRef.current || !styleReady) return;
    mapRef.current.easeTo({ center: [item.longitude, item.latitude], zoom: 17, duration: window.matchMedia('(prefers-reduced-motion: reduce)').matches ? 0 : 600 });
  }, [selected, styleReady]);
...
function Detail({item,onClose,onStatus,onPriority}:{item:QueueItem;onClose:()=>void;onStatus:(s:Status)=>void;onPriority:(p:number)=>void}) {
  ...
  streetView.search = new URLSearchParams({ api: '1', map_action: 'pano', viewpoint: `${item.latitude},${item.longitude}` }).toString();
  return <section className="detail" aria-label="Detalhe da ocorrência"><div className="detail-head"><div><span className={`priority-badge p${item.effective_priority}`}>{urgencyName(item.effective_priority)} · nível {item.effective_priority}</span>{item.is_test && <span className="pilot">OCORRÊNCIA DE TESTE</span>}<h2>{categoryName[item.category]}</h2><code>{item.protocol}</code></div><button className="close" onClick={onClose} aria-label="Fechar detalhe">×</button></div>
    ...
    <div className="street-view"><a href={streetView.toString()} target="_blank" rel="noopener noreferrer">🚶 Explorar a rua em primeira pessoa ↗</a><p>Abre o Google Street View próximo deste ponto. Clique nas setas para percorrer a rua. A cobertura e a data das imagens variam; o ponto exato da ocorrência continua indicado no mapa.</p></div>
