...
async function suggest(item) {
  ...
  try {
    ...
    if (!Number.isInteger(priority) || priority < 1 || priority > 5) throw new Error('Prioridade inválida devolvida pelo modelo');
...
const recoverSafely = () => catchUp().catch(error => console.error('[recuperação]', error.message));
...
supabase.channel('local-ai-triage')
  .on('postgres_changes', { event: '*', schema: 'public', table: 'occurrences' }, recoverSafely)
  .subscribe(status => console.log(`[conexão] ${status}`));

setInterval(recoverSafely, 5 * 60 * 1000);
